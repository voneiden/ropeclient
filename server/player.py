...
class PlayerManager(Database):
    def __init__(self, core=None, client=None):
        """
        PlayerManager provides interface for database actions involving
    ...
    def new(self, name, password, **kwargs):
        ...
        assert isinstance(player, Database)  # PyCharm assert hint

        # Update interfaces (we already verified the ident is unique above)
        self.interfaces[ident] = player
...
class Player(Database):
    def __init__(self, core=None, client=None, ident=None):
        """
        Player object provides interface to access a player entries
        in the database.
    ...
    def path(self, *args):
        ...
        if len(args) == 0:
            return "rp:players:{}".format(self.ident)

        elif len(args) == 1 and isinstance(args[0], str):
            return "rp:players:{}.{}".format(self.ident, args[0])
